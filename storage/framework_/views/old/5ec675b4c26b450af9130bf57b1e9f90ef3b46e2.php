...
<div class="row">
            ...
            <div class="col-md-4">
                    ...
                    <div class="card bg-c-green order-card">
                        <div class="card-block">
                            <h2 class="m-b-20">Total Participating SGC/As with Lab Results</h2>
                            <h1 class=""><i class="fa fa-users"></i><span>      <?php echo e($sg_list); ?> SGC/A's</span></h1>
                            <p class="m-b-20"> <h5> Total Cooperatives:     <?php echo e($total_coops); ?> SGC/A's  </h5> </p>
            ...
            <div class="col-md-4">
                ...
                <div class="card bg-c-pink order-card">
                    <div class="card-block">
                        ...
                        <h1 class=""><i class="fa fa-thumbs-o-down"></i><span>      <?php echo e($rejection_rate); ?> %</span></h1>
                        <p class="m-b-20"> <h5> Total Bag Tested:  <?php echo e(number_format($rejection_data->total_bags)); ?> bag(s)  </h5> </p>
            ...
            <div class="col-md-4">
                ...
                <div class="card bg-c-yellow order-card">
                    <div class="card-block">
                        <h2 id="synced_date">Synced Date : <?php echo e(date("F j, Y (g:i a)", strtotime($synced_data))); ?></h2>
                    ...
                    <?php foreach($regions as $region): ?>
                        <div class="card">
                            <div class="card-header" id="headingOne" >
                                ...
                                <h5 class="mb-0" style="margin:0;"  data-toggle="collapse" data-target="#collapse<?php echo e($region->id); ?>" aria-controls="<?php echo e($region->id); ?>" onclick="getCoopsDetails(<?php echo e($region->id); ?>, 'list_<?php echo e($region->id); ?>')" >
                                    <?php if($region->with_data == "true"): ?>
                                        <button style="color: #655f5f;text-decoration:none;width:98%; margin:0;text-align:left;" class="btn btn-link">
                                            <span class="badge badge-success"><i class="fa fa-users" aria-hidden="true"></i>
                                            </span>     <?php echo e($region->regDesc); ?>      
                                    ...
                                    <i class="fa fa-plus pull-right" id="icon_id_<?php echo e($region->id); ?>" style="margin-top: 12px;margin-right: 10px;" data-toggle="collapse" data-target="#collapse<?php echo e($region->id); ?>" aria-controls="<?php echo e($region->id); ?>" onclick="getCoopsDetails(<?php echo e($region->id); ?>, 'list_<?php echo e($region->id); ?>')"></i>
                                </h5>
                            ...
                            <div id="collapse<?php echo e($region->id); ?>" class="collapse" aria-labelledby="headingOne" data-parent="#accordion" style="margin-top: .5vw;">
                                <div class="card-body">
                                    <ul class="list-group row" style="width: 97%;margin-left: 1vw;" id="list_<?php echo e($region->id); ?>">
                                    </ul>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $("#exportAll").on('click', function (e){
        var url = '<?php echo e(route("rsis.export.all.rla")); ?>';
        window.open(url);
    });
    ...
    function sync_rla(){
        ...
        if(yesno){
            // HoldOn.open(holdon_options);
                    $.ajax({
                    type: 'POST',
                    url: "<?php echo e(route('coop.rla-report')); ?>",
                    data: {
                    ...
                    success: function (response, textStatus, request) {
                        $("#synced_date").empty().text(response);
                        HoldOn.close();  
                    }
                });
  ...
  $('#show_rla_modal').on('show.bs.modal', function (e) {
        ...
        $("#coop_rsis_rla").DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo e(route('rsis.view.coop.rla')); ?>",
                "dataType": "json",
                "type": "POST",
                "data":{
                    "_token": "<?php echo e(csrf_token()); ?>",
    ...
    function getCoopsDetails(region_id, list_id){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo e(route('rsis.get.rsis.rla')); ?>",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                region: region_id
            },
            success: function(data){
                $("#"+list_id).empty();
                //header
                $("#"+list_id).append("<li class = 'list-group-item col-xs-6'><strong>Seed Cooperative</strong></li>");
                $("#"+list_id).append("<li class = 'list-group-item col-xs-4'><strong>Total RLAs</strong></li>");
                ...
                jQuery.each(data, function(index, array_value){
                    //link
                    count = count + 1;
                   var view_btn = "<button class='btn btn-warning btn-sm' style='width: 10vw;'  data-toggle='modal' data-target='#show_rla_modal' data-account='"+array_value['account_no']+"' data-passed='"+array_value['passed']+"' data-rejected='"+array_value['rejected']+"' data-pending='"+array_value['pending']+"' data-coop_name='"+array_value['coop_name']+"' ><i class='fa fa-folder-open-o' aria-hidden='true' > View </i></button>";
                   var export_btn = "<button class='btn btn-success btn-sm' onclick='export_excel("+array_value['account_no']+")' style='width: 10vw;'><i class='fa fa-file-excel-o' aria-hidden='true' id='exp_"+array_value['account_no']+"'> Export </i></button>";
                    
           

                    //body
                    $("#"+list_id).append("<li class = 'list-group-item col-xs-6'  style='height:"+height_var+";'><big><strong>"+array_value['coop_name']+"</strong></big>"
                        +"<br>Acct #:<strong>"+array_value['coop_accre']+"</strong>"
                        +"<br>MOA #: <strong>"+array_value['moa_number']+"</strong>"
                        +"</li>");
                    
                    $("#"+list_id).append("<li class = 'list-group-item col-xs-4' style='height:"+height_var+";'>  <i class='fa fa-check-circle-o' aria-hidden='true'></i> Passed:<strong> "+array_value['passed']+" bag(s) </strong>"
                        +"<br> <i class='fa fa-times-circle-o' aria-hidden='true'></i> Rejected:<strong> "+array_value['rejected']+" bag(s)</strong>"
                        +"<br> <i class='fa fa-clock-o' aria-hidden='true'></i> Pending:<strong> "+array_value['pending']+" bag(s)</strong>"
                        +"</li>");
                    $("#"+list_id).append("<li class = 'list-group-item col-xs-2'  style='height:"+height_var+";'><center>"+view_btn+export_btn+"</center></li>");
                });
    ...
    function exportindexOfPayment(coop_accreditation, button_id){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo e(route('delivery_dashboard.export_deliveries.iop')); ?>",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
    ...
    function exportDeliveries(coop_accreditation, button_id){
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo e(route('delivery_dashboard.export_deliveries')); ?>",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                coop_accreditation: coop_accreditation
            },
    ...
    $('#show_coop_batches_modal').on('show.bs.modal', function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo e(route('delivery_dashboard.coop.name')); ?>",
            data: {
                _token: "<?php echo e(csrf_token()); ?>",
                coop_id: coop_id
            },
        ...
        $("#coop_batch_table").DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo e(route('delivery_dashboard.batch.list')); ?>",
                "dataType": "json",
                "type": "POST",
                "data":{
                    "_token": "<?php echo e(csrf_token()); ?>",
