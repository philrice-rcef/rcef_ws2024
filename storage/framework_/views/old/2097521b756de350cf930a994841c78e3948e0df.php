...
<div class="row">
    ...
    <div class="col-md-12">
                <h2>Delivery vs Allocation (Municipal Level)</h2>
            ...
            <div class="x_title">
                ...
                <select name="province_allocation" id="province_allocation" class="form-control">
                    ...
                    <?php endforeach; ?>
                        ...
                        <br>
                <select name="municipality_allocation" id="municipality_allocation" class="form-control">
...
<?php $__env->startPush('scripts'); ?>
    ...
    function loadChart() {
        ...
        var municipality = $('select[name="municipality_allocation"]').val();
            if(municipality === "0"){
                municipality = "all";
            }
        ...
        var level = "municipal";
         $.ajax({
                    type: 'POST',
                    url: "<?php echo e(route('delivery.allocation.chart')); ?>",
                    data: {
    ...
    function loadTBl(){
        ...
        //alert(province+" "+municipality);
            ...
            $('#tblAllocation').DataTable({
                ...
                "pageLength": 25,
                "ajax": {
                    "url": "<?php echo e(route('delivery.allocation.gentable')); ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data":{
                        "_token": "<?php echo e(csrf_token()); ?>",
        ...
        var province = $('select[name="province_allocation"]').val();
            $.ajax({
                method: 'POST',
                url: "<?php echo e(route('delivery.allocation.muncipality')); ?>",
                data: {
                    _token: _token,
                    province: province
                },
                ...
                success: function (source) {
                    $('select[name="municipality_allocation"]').empty().append('<option value="0">Please select a Municipality</option>');
                $.each(source, function (i, d) {
                    if(i == 0){
                        $('select[name="municipality_allocation"]').empty().append('<option value="all">--ALL MUNICIPALITY--</option>');
                    }

                    $('select[name="municipality_allocation"]').append('<option value="' + d.municipality + '">' + d.municipality + '</option>');
                });
...
$('select[name="municipality_allocation"]').on('change', function () {
    HoldOn.open(holdon_options);
        loadTBl();
        loadChart();
    HoldOn.close();
