...
class virtual_encodingController extends Controller
{
    ...
    public function index_homeAddressClaim()
    {
        ...
        if (Auth::user()->roles->first()->name == "rcef-programmer") {
            ...
            $provinceList = ["ABRA", "CAGAYAN", "CAMARINES SUR", "IFUGAO", "ISABELA", "KALINGA", "NUEVA VIZCAYA", "QUIRINO", "ILOILO", "NEGROS OCCIDENTAL", "BOHOL", "CAPIZ", "SAMAR (WESTERN SAMAR)", "LEYTE", "CAMARINES SUR", "MASBATE", "ANTIQUE"];
            $user_provinces = DB::table($GLOBALS['season_prefix'] . 'rcep_delivery_inspection.lib_prv')
                ->select('lib_prv.province')
            ...
            foreach ($request->distribution as $key => $release) {
                if ($key == 0) {
                    ...
                    if ($farmer != null) {
                        ...
                        $ayuda = "";
                        if ($ayuda_fertilizer == "true") {
                            $ayuda .= "fertilizer";
                        }
                        if ($ayuda_incentives == "true") {
                            $ayuda .= ",cash_incentives";
                        }
                        if ($ayuda_credit == "true") {
                            $ayuda .= ",credit";
                        }
                        $is_representative = 0;
                        if ($request->rep) {
                ...
                foreach ($request->distribution as $key => $release) {
                    if ($key > 0) {
                        ...
                        $seed_stock = DB::connection('delivery_inspection_db')->table('tbl_actual_delivery')
                            ...
                            ->sum("totalBagCount");
                        if ($seed_stock < $bags_claimed) {
                            return json_encode("Seed stocks exhausted");
                        }
