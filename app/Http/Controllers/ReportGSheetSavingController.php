...
class ReportGSheetSavingController extends Controller
{
    ...
    public function saveNewDS2021(Request $request){
        ...
        try {
            ...
            $data = array(
                ...
                "from_dop" => "N/A",
                "from_bags_total" => $request->from_bags_total,
            ...
            DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                ...
                ->update(
                    [
                        "current_balance" => $balance_details->current_balance + $request->from_bags_total,
                    ]
                );
            ...
            $logs = array(
                ...
                "total_bags" => $request->from_bags_total,
                "after_balance" => $balance_details->current_balance + $request->from_bags_total,
                "remarks" => "ADDED: new seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s)",
    ...
    public function saveInventory_warehouse(Request $request){
        ...
        try {
            ...
            if($balance->current_balance >= $request->from_bags_total){
                ...
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                    ...
                    ->update(
                        [
                            "current_balance" => $balance->current_balance - $request->from_bags_total,
                        ]
                    );       
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $balance->current_balance - $request->from_bags_total,
                    "remarks" => "DEDUCT: seeds from $request->from_province, $request->from_municipality ($request->from_bags_total) bag(s) | PHILRICE WAREHOUSE"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
                ...
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                    ...
                    ->update(
                        [
                            "current_balance" => $to_balance->current_balance + $request->from_bags_total,
                        ]
                    );       
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $to_balance->current_balance + $request->from_bags_total,
                    "remarks" => "ADDED: seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s) | PHILRICE WAREHOUSE"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
    ...
    public function saveInventory_lgu(Request $request){
        ...
        try {
            ...
            $logs = array(
                ...
                "remarks" => "RECORDED: seeds to $request->from_province, $request->from_municipality ($request->from_bags_total) bag(s) | LGU STOCKS"
            );
            DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
    ...
    public function saveInventory_transferred(Request $request){
        ...
        try {
            ...
            if($from_balance->current_balance >= $request->from_bags_total){
                ...
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                    ...
                    ->update(
                        [
                            "current_balance" => $from_balance->current_balance - $request->from_bags_total,
                        ]
                    );       
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $from_balance->current_balance - $request->from_bags_total,
                    "remarks" => "DEDUCT: seeds from $request->from_province, $request->from_municipality ($request->from_bags_total) bag(s) | TRANSFERRED SEEDS"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $to_balance->current_balance + $request->from_bags_total,
                    "remarks" => "ADDED: seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s) | TRANSFERRED SEEDS"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
    ...
    public function editNewDS2021(Request $request){
        $this->validate($request, [
            ...
            'from_bags_total' => 'required|not_in:0'
        ]);
        ...
        try {
            ...
            $old_schedule_details = DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_schedule')
                ->select(DB::raw('SUM(from_bags_total) as total_bags'), 'from_province', 'to_province',
            ...
            DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                ...
                ->update(
                    [
                        "current_balance" => $to_balance_after->current_balance + $request->from_bags_total,
                    ]
                );       
            ...
            $logs = array(
                ...
                "total_bags" => $request->from_bags_total,
                "after_balance" => $to_balance_after->current_balance + $request->from_bags_total,
                "remarks" => "ADDED: seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s) | NEW SEEDS | EDIT"
            );
            DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
    ...
    public function editInventoryWarehouse(Request $request){
        ...
        try {
            ...
            if($from_balance_after->current_balance >= $request->from_bags_total){
                $data = array(
                    "transaction_title" => $request->title_transaction,
                    "transaction_code" => $transaction_code,
                    "seed_type" => $request->seed_type,
                ...
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_seed_balance')
                    ...
                    ->update(
                        [
                            "current_balance" => $from_balance_after->current_balance - $request->from_bags_total,
                        ]
                    );       
                $logs = array(
                    "station_id" => Auth::user()->stationId,
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $from_balance_after->current_balance - $request->from_bags_total,
                    "remarks" => "DEDUCT: seeds from $request->from_province, $request->from_municipality ($request->from_bags_total) bag(s) | PHILRICE WAREHOUSE | EDIT"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $to_balance->current_balance + $request->from_bags_total,
                    "remarks" => "ADDED: seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s) | PHILRICE WAREHOUSE | EDIT"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
    ...
    public function editInventoryTransferred(Request $request){
        ...
        try {
            ...
            if($current_balance_after->current_balance >= $request->from_bags_total){
                $data = array(
                    "transaction_title" => $request->title_transaction,
                    "transaction_code" => $transaction_code,
                    "seed_type" => $request->seed_type,
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $from_balance->current_balance - $request->from_bags_total,
                    "remarks" => "DEDUCT: seeds from $request->from_province, $request->from_municipality ($request->from_bags_total) bag(s) | TRANSFERRED SEEDS | EDIT"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
                ...
                $logs = array(
                    ...
                    "total_bags" => $request->from_bags_total,
                    "after_balance" => $to_balance->current_balance + $request->from_bags_total,
                    "remarks" => "ADDED: seeds to $request->to_province, $request->to_municipality ($request->from_bags_total) bag(s) | TRANSFERRED SEEDS | EDIT"
                );
                DB::table($GLOBALS['season_prefix'].'rcep_google_sheets.lib_balance_logs')->insert($logs);
