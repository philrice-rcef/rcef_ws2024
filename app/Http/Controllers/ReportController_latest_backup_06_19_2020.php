...
class ReportController extends Controller
{
    ...
    function generate_regional_data(){
        ...
        $region_prv = DB::connection('delivery_inspection_db')->table('lib_prv')->groupBy('region')->get();
            ...
            if($table_conn == "Connection Established!"){
                ...
                if(count($prv_dist_data) > 0){
                    ...
                    ->insert([
                        ...
                        'total_dist_area'   => $report_res->dist_area,
                        'total_actual_area' => $report_res->actual_area,
                        'total_male'        => $male_count,
                        'total_female'      => $female_count,
                        'date_generated'    => date("Y-m-d H:i:s")
                    ]);
    ...
    public function convert_to_excel_province($province){
        ...
        if($table_conn == "Connection Established!"){
            ...
            if(count($prv_dist_data) > 0){
                ...
                ->select('farmer_profile.firstName','farmer_profile.midName','farmer_profile.lastName',
                        'farmer_profile.extName','farmer_profile.sex','farmer_profile.rsbsa_control_no',
                        'farmer_profile.area', 'farmer_profile.actual_area', 
                        'released.province', 'released.municipality', 'released.seed_variety', 
                        'released.bags', 'released.date_released', 'released.farmer_id',
