...
class CheckingController extends Controller {
        ...
        return Datatables::of($table_data)
                ->addColumn('action', function($table_data) {
                    return '<a for="' . $table_data['rsbsa_id'] . '" class="btn btn-danger btn-xs deleteDatacheck"><i class="fa fa-trash-o"></i> Delete </a><br>
                            <a href="#" data-rsbsa="'.$table_data['rsbsa'].'" data-toggle="modal" data-target="#edit_check_modal" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i> Edit Details</a>';
                })
                ->make(true);
    }
    ...
    public function search(Request $request) {
        ...
        return Datatables::of($table_data)
                        ->addColumn('action', function($table_data) {
                             /*return ((Auth::user()->roles->first()->name == "system-encoder") ?  "" :'<a for="' . $table_data['rsbsa_id'] . '" class="btn btn-danger btn-xs deleteDatacheck"><i class="fa fa-trash-o"></i> Delete </a><br>
							 <a href="#" data-rsbsa="'.$table_data['rsbsa'].'" data-toggle="modal" data-target="#edit_check_modal" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i> Edit Details</a>');*/
							 if(Auth::user()->roles->first()->name == "system-encoder"){
								return '<a href="#" data-rsbsa="'.$table_data['rsbsa_id'].'" data-toggle="modal" data-target="#edit_check_modal" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i> Edit Details</a>';
							 }else{
								return '<a for="' . $table_data['rsbsa_id'] . '" class="btn btn-danger btn-xs deleteDatacheck"><i class="fa fa-trash-o"></i> Delete </a><br>
								<a href="#" data-rsbsa="'.$table_data['rsbsa_id'].'" data-toggle="modal" data-target="#edit_check_modal" class="btn btn-warning btn-xs"><i class="fa fa-edit"></i> Edit Details</a>';
							 }
                        })
                        ->make(true);
    }
        ...
        try {
            ...
            if($pending_data->created_by == Auth::user()->username){
                ...
                ->insert([
                    ...
                    'update_description' => "updated RSBSA # to: `".$request->new_rsbsa."` & Middle Name: ".$request->middle_name,
                    'performed_by' => Auth::user()->username
                ]);
