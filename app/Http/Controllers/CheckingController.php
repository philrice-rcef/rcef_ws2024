...
class CheckingController extends Controller {
    ...
    public function get_farmer_data_forEdit(Request $request){
        ...
        if(count($pending_data) > 0){
            ...
            $contact_no = DB::table($prv_code.".other_info")->where('rsbsa_control_no', $request->current_rsbsa)->value('phone');
    ...
    public function update_farmer_data_forEdit_old(Request $request){
        ...
        try {
            ...
            ->insert([
                ...
                'update_description' => "updated RSBSA # to: `".$request->new_rsbsa."` [DS 2019 DATA]"."` & Middle Name: ".$request->middle_name.", First Name: ".$request->first_name.", Last Name: ".$request->last_name.", Ext Name: ".$request->ext_name.", Birth Date: ".$request->birth_date.", Sex: ".$request->sex.", Contact #: ".$request->contact_no,
                'performed_by' => Auth::user()->username
            ]);
    ...
    public function update_farmer_data_forEdit(Request $request){
        ...
        try {
            ...
            if(count($pending_data) > 0){
                if($pending_data->created_by == Auth::user()->username || Auth::user()->roles->first()->roleId == 1 && substr($pending_data->prv_dropoff_id, 0, 4) == Auth::user()->province){
                    ...
                    ->insert([
                        'original_rsbsa' => $request->old_rsbsa,
                        'update_description' => "updated RSBSA # to: `".$request->new_rsbsa."` & Middle Name: ".$request->middle_name.", First Name: ".$request->first_name.", Last Name: ".$request->last_name.", Ext Name: ".$request->ext_name.", Birth Date: ".$request->birth_date.", Sex: ".$request->sex.", Contact #: ".$request->contact_no,
                        'performed_by' => Auth::user()->username
                    ]);
					
					/**
                     * UPDATE MIRROR FARMER BENEFICIARY LIST ALSO
                     */
                    $rpt_database     = "rpt_".Auth::user()->province;
                    $rpt_tbl_database = "tbl_".substr($pending_data->prv_dropoff_id, 0, 6);
                    DB::table($rpt_database.".".$rpt_tbl_database)
                        ->where('rsbsa_control_number', '=', $request->old_rsbsa)
                        ->update(
                            [
                                //'rsbsa_control_no' => $request->new_rsbsa,
                                //'firstName' => $request->first_name,
                                ...
                                'rsbsa_control_number' => $request->new_rsbsa,
                                'farmer_fname' => $request->first_name,
                                'farmer_mname' => $request->middle_name,
                                'farmer_lname' => $request->last_name,
                                'farmer_ext' => $request->ext_name,
                                'sex' => $request->sex,
                                'birthdate' => $request->birth_date,
                                'tel_number' => $request->contact_no
                            ]
                        );

                    DB::table('rpt_logs.update_logs')
                    ->insert([
                        'old_rsbsa_number' => $request->old_rsbsa,
                        'new_rsbsa_number' => $request->new_rsbsa,
                        'farmer_fname' => $request->first_name,
                        'farmer_mname' => $request->middle_name,
                        'farmer_lname' => $request->last_name,
                        'farmer_ext' => $request->ext_name,
