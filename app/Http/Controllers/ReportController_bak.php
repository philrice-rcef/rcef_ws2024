...
class ReportController extends Controller
{
    ...
    public function generate_national_info($key){
        DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_national_reports')->truncate();
        
        $prv_db_list = DB::table("information_schema.TABLES")
        ...
        foreach ($prv_db_list as $key => $value) {
            
            $released_data = DB::table($value->TABLE_SCHEMA.".released")
                ->get();
                if(count($released_data)>0){
                    $ntl_province++;
                    $region_code = substr(str_replace("prv_", "", $value->TABLE_SCHEMA), 0,2);
                    if($region_code != $reg_data){
                        $reg_data = $region_code;
                        $ntl_region++;
                    }

                    $ntl_municipalities += DB::table($value->TABLE_SCHEMA.".released")
                        ->groupBy("municipality")
                        ->count("municipality");

                        //I STOPPED HERE
                    $ntl_farmers += DB::table($value->TABLE_SCHEMA.".released")
    ...
    function generate_all_data_10062021(){
        DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_national_reports')->truncate();
        DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_regional_reports')->truncate();
        DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_provincial_reports')->truncate();
        DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_municipal_reports')->truncate();
        ...
        $provinces = DB::table($GLOBALS['season_prefix'].'rcep_reports.lib_municipal_reports')
            ->select('region','province',DB::raw('SUM(total_farmers) as total_farmers'),
                ...
                DB::raw('SUM(total_female) as total_female'),
				DB::raw('SUM(yield) as total_yield'),
				DB::raw('SUM(farmers_with_yield) as total_farmers_with_yield'), DB::raw('SUM(total_claimed_area) as claimed_area'))
            ->where('total_bags', '!=', 0)
            ->groupBy('province')
            ->orderBy('report_id')
            ->get();
        ...
        foreach($prv_list as $prv_database){
                ...
                if(count($prv_dist_data) > 0){
                    $variety_res = DB::table($prv_database.".released")
                        ->select(DB::raw('SUM(released.bags) as total_seed_bags'),'seed_variety', 'province', 'municipality',DB::raw('SUM(released.claimed_area) as total_area'),DB::raw('COUNT(released.rsbsa_control_no) as total_farmers') )
    ...
    public function Home_Report2(){
        ...
        if(Auth::user()->roles->first()->name == "da-icts"){
            ...
            foreach ($excel_data as $key => $value) {
                ...
                $malecount = "<div><strong> Male: ".number_format($excel_data[$key]['total_male'])."</strong></div>";
                $per_province= $this->area_range_counter("",  $excel_data[$key]['region'], "%", "%", "Male"); 
                ...
                $excel_data[$key]['total_male'] =  $malecount;

                $femalecount = "<div> <strong> Female: ".number_format($excel_data[$key]['total_female'])."</strong> </div>";
                $per_province= $this->area_range_counter("",  $excel_data[$key]['region'], "%", "%", "Female"); 
                if(count($per_province)>0){
                ...
                $excel_data[$key]['total_female'] =  $femalecount;



                $excel_data[$key]['linkBreakDown'] = $accepted_transferred;
            }
   ...
   public function Home_Report2_municipal(){
    ...
    public function Home_Report2_national(){
        ...
        $provinces_allocated = DB::table($GLOBALS['season_prefix']."rcep_delivery_inspection.lib_target_datasets")
            ->where("area_coverage", ">", 0)
            ->groupBy("province")
            ->get();
            $total_yield = 0;
            foreach ($provinces_allocated as $allocated) {
                $total_yield += DB::table($GLOBALS['season_prefix']."rcep_reports.lib_provincial_reports")->where("province", $allocated->province)->sum("yield");
            }


            if(count($provinces_allocated)>0){             
                $yield_of_allocated_provinces = $total_yield / count($provinces_allocated);
            }else{
                $yield_of_allocated_provinces = 0;
            }
        ...
        if(Auth::user()->roles->first()->name == "da-icts"){
            ...
            return view('reports.modified.national')
                ...
                ->with('yield_allocated', $yield_of_allocated_provinces);
        }
    }
