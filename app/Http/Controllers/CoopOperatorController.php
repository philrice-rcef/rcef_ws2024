...
class CoopOperatorController extends Controller
{
    public function coop_delivery_home(){
        ...
        $varieties = $this->get_coop_commited_varieties();
    ...
    public function coop_total_values(Request $request){
        ...
        $commitments = DB::connection('seed_coop_db')->table('tbl_commitment')->where('coopID', $coop_id)->groupBy("commitment_variety")->get();
	...
	**/
	...
	public function SGEnrollment(){
		...
		$total_sg_enrolled = count(DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives_members')->where(['is_active' => 0, 'coop_accreditation_number' => $tagged_accreditation])->get());
		$total_sg_area = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives_members')->where(['is_active' => 0, 'coop_accreditation_number' => $tagged_accreditation])->sum('area');
	...
	public function SGMatrix(Request $request){
		...
		$target_efficiency = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $tagged_accreditation)->value('target_efficiency');
        $bags_ha = intval($target_efficiency) * 200 / 100;
		$required_commitments = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment as a')
            ->select(DB::raw('SUM(commitment_value) as total_value'), 'commitment_variety')
            ->addSelect(DB::raw("(
                SELECT (SUM(allocated_area) * $bags_ha) from rcep_seed_cooperatives.tbl_commitment_member b
                    WHERE b.coop_accreditation_number = '$tagged_accreditation'
                    AND b.commitment_variety = a.commitment_variety
            ) as allocated_seed"))
			->where('a.coopID', $coop_id)
			->where('a.variety_status', '!=', 0)
			->groupBy('a.commitment_variety')
			->get();
			
		return view('dashboard.coop_operator.matrix')
			->with('required_commitments', $required_commitments)
			->with('bags_ha', $bags_ha)
			->with('tagged_accreditation', $tagged_accreditation);
	}

    public function SGMatrixVarieties(Request $request){
		$tagged_accreditation = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
		$coop_id = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $tagged_accreditation)->value('coopId');
		$target_efficiency = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $tagged_accreditation)->value('target_efficiency');
        $bags_ha = intval($target_efficiency) * 200 / 100;
		...
		return view('dashboard.coop_operator.matrix_varieties')
			->with('required_commitments', $required_commitments)
			->with('bags_ha', $bags_ha)
			->with('tagged_accreditation', $tagged_accreditation);
	}
	...
	public function SGGetVariety(Request $request){
			...
			$tbl_commitment = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment')
                ...
                $target_efficiency = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $request->coopaccre)->value('target_efficiency');
                $bags_ha = intval($target_efficiency) * 200 / 100;
				$ret = array();
				foreach ($tbl_commitment as $commitment){
                    ...
                    if(count($checkAllMemberCommitment)>0){
                        $allMemberCommitment = $checkAllMemberCommitment * intval($bags_ha);
    ...
    public function get_coop_commited_varieties(){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $varieties = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional')
                ->select('seed_variety')
    ...
    public function get_coop_commited_region(){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
    ...
    // tim added functions 
    public function coop_commitment_variety(Request $request){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $regions = $this->get_coop_commited_region();
        $commitments = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional as a')->select('*')->addSelect(DB::raw("SUM(volume) as volume_commitment"));
        $region_col_no = 0;
        foreach ($regions as $region){
            $region_col_no++;
            $commitments->addSelect(DB::raw("(
                SELECT COALESCE(SUM(volume), 0) from rcep_seed_cooperatives.tbl_commitment_regional as b
                    WHERE region_name = '$region->region_name'
                    AND accreditation_no = '$coop_accre'
                    AND b.seed_variety = a.seed_variety
            ) region_col_no_$region_col_no"));
        }

        $committed = $commitments->where(["accreditation_no" => $coop_accre])
                ->groupBy('seed_variety');
                

        return Datatables::of($committed)
        ->addColumn('action', function($row) {
            return "<a type='button' class='btn btn-primary btn-sm' href='#' data-toggle='modal' data-target='#sg_commitment_modal' data-variety='$row->seed_variety' id='view_sg'><i class='fa fa-eye'></i> SG Allocated</a>";
        })
        ->make(true);
        // return json_encode($results);
    }

    public function sg_commitment_variety_html(Request $request){
        return view('dashboard.coop_operator.sg_variety_table');
    }

    public function sg_commitment_variety_table(Request $request){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $target_efficiency = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $coop_accre)->value('target_efficiency');
        $bags_ha = intval($target_efficiency) * 200 / 100;
        $commitments = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_member')
                ->select('*')
                ->addSelect(DB::raw("CONCAT(first_name,' ',middle_name,' ',last_name) as full_name"))
                ->addSelect(DB::raw("allocated_area * $bags_ha as target_volume"))
                ->where([
                    "coop_accreditation_number" => $coop_accre,
                    "commitment_variety" => $request->variety,
                ]);
    ...
    public function dropoff_table(Request $request){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $target_efficiency = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $coop_accre)->value('target_efficiency');
        $varieties = $this->get_coop_commited_varieties();
        ...
        foreach ($varieties as $variety){
            $variety_col_no++;
            $dop->addSelect(DB::raw("(
                SELECT COALESCE(SUM(totalBagCount), 0) from rcep_delivery_inspection.tbl_delivery
                    WHERE seedVariety = '$variety->seed_variety'
                    AND coopAccreditation = '$coop_accre'
                    AND lib_dropoff_point.prv = tbl_delivery.prv
                    AND is_cancelled = 0
            ) variety_col_no_$variety_col_no"));
        }
        ...
        $dop = $dop->where([
            "coop_accreditation" => $coop_accre,
            "is_active" => 1
            ])
            ->groupBy('lib_dropoff_point.prv');        
        
                
        return Datatables::of($dop)
        ->addColumn('action', function($row) {
            return "<a type='button' class='btn btn-primary btn-sm' href='#' data-toggle='modal' data-target='#dop_di_modal' data-prv='$row->prv' id='view_dop'><i class='fa fa-eye'></i> DOP </a>
                    <a type='button' class='btn btn-primary btn-sm' href='#' data-toggle='modal' data-target='#dop_di_modal' data-prv='$row->prv' id='view_batch'><i class='fa fa-eye'></i> Batch no.</a>";
        })
        ->make(true);
    }

    public function confirmed_dop_html(Request $request){
        $confirmed = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.lib_dropoff_point')
            ->select('*')
            ->where([
                "prv" => $request->prv,
            ])
        ...
        return view('dashboard.coop_operator.confirm_dop_table')
                ->with('region', $confirmed->region)
                ->with('province', $confirmed->province)
                ->with('municipality', $confirmed->municipality);
    }

    public function confirmed_dop_table(Request $request){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $moa = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $coop_accre)->value('current_moa');
        
        $confirmed = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_delivery')
                ->select(['tbl_delivery.*'])
                ->addSelect(DB::raw("COALESCE(SUM(tbl_delivery.totalBagCount), 0) as confirmed"))
                ->addSelect(DB::raw("COALESCE(SUM(tbl_actual_delivery.totalBagCount), 0) as inspected"))
                ->leftjoin($GLOBALS['season_prefix']."rcep_delivery_inspection.tbl_actual_delivery", function($join) use ($moa){
                    $join->on("tbl_actual_delivery.prv_dropoff_id", "=", "tbl_delivery.prv_dropoff_id")
                        ->on("tbl_actual_delivery.seedTag", "=", "tbl_delivery.seedTag")
                        ->where("tbl_actual_delivery.moa_number", "=", $moa)
                        ->where("tbl_actual_delivery.is_transferred", "=", 0);
                })
                ->where([
                    "coopAccreditation" => $coop_accre,
                    "tbl_delivery.prv" => $request->prv,
                    "is_cancelled" => 0,                   
                ])
                ->groupBy('tbl_delivery.prv_dropoff_id');
                
        return Datatables::of($confirmed)
        ->addColumn('percentage', function($row) {
            $percentage = 100 * intval($row->inspected) / intval($row->confirmed);
            return number_format($percentage, 2) .' %';
        })
        ->make(true);
    }

    public function confirmed_batch_html(Request $request){
        $confirmed = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.lib_dropoff_point')
            ->select('*')
            ->where([
                "prv" => $request->prv,
        ...
        return view('dashboard.coop_operator.confirm_batch_table')
                ->with('region', $confirmed->region)
                ->with('province', $confirmed->province)
                ->with('municipality', $confirmed->municipality);
    }

    public function confirmed_batch_table(Request $request){
        $coop_accre = DB::table($GLOBALS['season_prefix'].'sdms_db_dev.users_coop')->where('userId', Auth::user()->userId)->value('coopAccreditation');
        $moa = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_cooperatives')->where('accreditation_no', $coop_accre)->value('current_moa');
