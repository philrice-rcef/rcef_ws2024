...
//        $check_pending = $farmer->get_farmer_rsbsa($rsbsa_control_no);
    ...
    public function formula($actual_area, $flag) {
        ...
        return ($flag == 1 ? $return_area : $whole_bags + $plus);
    }
...
//        }
        ...
        $remaining = $this->formula($input['farm_area'], 2);
...
//            $remaining_bags = 0;
    ...
    public function add_farmer_rsbsa(Request $request) {
        ...
        $area = $this->formula($actual_area, 1);
        $bags_limit = $this->formula($actual_area, 2);
...
// Add farmer's rsbsa control number
            ...
            if ($result['farmerID']) {
                $farmer->add_other_info($rsbsa_control_no, $result['farmerID'], $birthdate, $mfname, $mmname, $mlname, $mextname, $valid_id, $relationship, $is_representative, $have_pic);
            }
        }
