...
class StationReportController extends Controller
{
    ...
    public function load_regional_report_values(Request $request){
        ...
        if($category == "REGION"){
            ...
            $total_provinces = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_province')
                ->where('region', $id)
                ->groupBy('province')
                ->get();

            $total_municipalities = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery')
                ->where('region', $id)
                ->groupBy('province', 'municipality')
                ->get();

            $confirmed_deliveries = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_delivery')
            ...
            $transferred_bags = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery')
                ->select(DB::raw('SUM(totalBagCount) as total_bags'), 'batchTicketNumber')
                ->where('region', $id)
            ...
            foreach($confirmed_deliveries as $row){
                $check_actual_delivery = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery')
            ...
            foreach ($prv_code as $key => $value) {
                if($prv_code != null ){
                    ...
                    // $farmer_beneficiaries += DB::table($prv_db. '.released')
                    //     ->groupBy('farmer_id')
                    //     ->count();
                    ...
                    $farmer_beneficiaries +=  count($tsf);
                }
            }
            ...
            // $area = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitmet_per_province')->where('id', $id)->first();
            $total_provinces = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery')
    ...
    public function load_seed_coopList(Request $request){
        ...
        if($category == "REGION"){
            $cooperatives = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_delivery')
                ...
                ->where('rcep_delivery_inspection.tbl_delivery.region', $id)
            ...
            $cooperatives = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_delivery')
                ...
                ->where('rcep_delivery_inspection.tbl_delivery.province', $id)
                ->groupBy('rcep_seed_cooperatives.tbl_cooperatives.accreditation_no')
                ->get();
        }
    ...
    function load_station_progress(Request $request){
        ...
        $regions_covered = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_allocation')->where('station_id', $request->station)->groupBy('region')->get();
        ...
        foreach($regions_covered as $row){
            ...
            //target data
            $target = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_allocation')->where('region', $row->region)->get();
            foreach($target as $t_row){
                $target_bags += $t_row->allocation;
            }

            $target_provinces += count(DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_allocation')->where('region', $row->region)->groupBy('province')->get());
            ...
            foreach ($prv_code as $key => $value) {
                if($prv_code != null ){
                    ...
                    $total_distributed += DB::table($prv_db. '.released')
                        // ->groupBy('farmer_id')
                        ->sum('bags');
		...
		$total_report_beneficiaries = 0;
		$reported_distributed = 0;
        return array(
