        ...
        foreach($batch_deliveries as $batch_row){
            ...
            $is_transfer_N = array();
             ...
             if($wholeCount > 0 ){
                ...
                $nextSeason = DB::table('ds2025_rcep_delivery_inspection.tbl_actual_delivery')
                        ->where('remarks', 'like', '%'.$batch_row->batchTicketNumber.'%')
                        ->where('seedVariety', $batch_row->seedVariety)
                        ->where('is_transferred', 1)
                        ->where('transferCategory', 'P')
                        ->where('seedTag', 'like', '%'.$batch_row->seedTag.'%')
                        ->first();

                        if($nextSeason)
                        {
                            $label = "Transferred ".$nextSeason->totalBagCount." bags to next season. New Batch Ticket Number: ".$nextSeason->batchTicketNumber.". Province: ".$nextSeason->province.". Municipality: ".$nextSeason->municipality;
                        }
                        else
                        {
                            $label = $label;
