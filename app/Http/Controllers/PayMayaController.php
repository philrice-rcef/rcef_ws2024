...
class PayMayaController extends Controller
{
            ...
            public function ebinhiCss(){
                ...
                $returns = DB::table($GLOBALS['season_prefix']."rcep_css.ebinhi_response")
                ->select( "philrice_id", "rcef_id", "tbl_beneficiaries.lastName","ebinhi_response.province", "ebinhi_response.municipality", "barangay","sex","q_1", "q_2", "q_3", "q_4", "q_5", "q_6", "q_7", "q_8", "q_9", "q_10", "q_11", "q_12", "q_13", "q_14", "q_15", "q_16")
                    ->join($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries","tbl_beneficiaries.paymaya_code","=","ebinhi_response.rcef_id")    
                ...
                foreach($returns as $ins){
                    ...
                    array_push($excel_data, array(
                        "philrice_id" => $ins->philrice_id,
                        "rcef_id" => $ins->rcef_id,
                        "province" => $ins->province,
                        "municipality" => $ins->municipality,
...
// "qr_code" => $row->qr_code, "seedTag" => $row->seedTag, "seedVariety" => $row->seedVariety,
    ...
    public function beneficiary_report_home(){
        ...
        foreach($provincial_data as $row){
            ...
            $claim_area =  DB::table($GLOBALS['season_prefix']."rcep_paymaya.paymaya_claim_area")->where('province', $row->province)->sum("sum(area)");
