...
class DeliveryDashboardController extends Controller
{
    ...
    public function get_coops($id) {
        $coop = \DB::connection('seed_coop_db')
                ...
                ->where('tbl_cooperatives.regionId', $id)
                ->orderBy('tbl_commitment_regional.coop_name')
                ->groupBy('tbl_commitment_regional.coop_Id')
                ->get();
        return $coop;
    }
    ...
    public function getCoop_in_region(Request $request){
       ...
       // dd($coop_list)
        ...
        foreach($coop_list as $row){
            $total_commitment = DB::connection("seed_coop_db")->table("tbl_commitment_regional")->where("coop_Id",  $row->coopId)->sum("volume");
            ...
            $coop_confirmed_replacement = DB::connection('delivery_inspection_db')->table("tbl_delivery")
                ->select(DB::raw('SUM(tbl_delivery.totalBagCount) as total_confirmed'))
                ->where('tbl_delivery.is_cancelled', '=', '0')
                ->where('tbl_delivery.coopAccreditation', '=', $row->accreditation_no)
                ->where("tbl_delivery.isBuffer", 9)
            ...
            $coop_delivery_batches_replacement = DB::connection('delivery_inspection_db')->table("tbl_delivery")
                ->select('batchTicketNumber', 'seedVariety')
                ->where('tbl_delivery.is_cancelled', '=', '0')
                ->where('tbl_delivery.coopAccreditation', '=', $row->accreditation_no)
                ->where("tbl_delivery.isBuffer", 9)
            ...
            foreach($coop_delivery_batches_replacement as $batch_row){
            ...
            $coop_arr = array(
                ...
                "total_commit" => number_format($total_commitment),
                "total_confirmed" => number_format($coop_confirmed_deliveries),
                "total_inspected" => number_format($total_inspected),
                "confirmed_replacement" => number_format($coop_confirmed_replacement)
            );
...
/*
    ...
    $prevBatch = DB::table($GLOBALS['season_prefix'].'rcep_transfers_ps.transfer_logs')
        ...
        $myFile = Excel::create('SEED_COOP_DELIVERIES', function($excel) use ($return_arr, $replacement_arr, $buffer_arr) {
