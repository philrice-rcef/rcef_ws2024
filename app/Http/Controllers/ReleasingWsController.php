...
use Auth;
use App\FarmerWs;
use App\InspectedSeedsWs;
use App\ReleasingWs;
use App\RegionsWs;
use App\Seeds;
use App\ProvincesWs;
use App\MunicipalitiesWs;
use App\DropoffPointsWs;

class ReleasingWsController extends Controller {


    public function pageClose(){
        $mss = "Please use the mobile app for encoding.";
    ...
    public function index() {
        ...
        if (Session::has('dropoff_point')) {
            ...
            $inspected_seeds = new InspectedSeedsWs();
            $seeds = new Seeds();
        ...
        $releasing = new ReleasingWs();
        $check_direct_seeded = $releasing->check_direct_seeded();
            return view('releasing_ws.index')
            ...
            // Get Provinces
            $provinces = new ProvincesWs();
            $provinces_list = $provinces->delivery_Provinces();

            return view('releasing_ws.login')->with(compact('provinces_list'));
        }
    }

    public function get_municipalities($province) {
        // Get municipalities
        $municipalities = new MunicipalitiesWs();
    ...
    public function get_dropoff_points($province, $municipality) {
        // Get dropoff points
        $dropoff_points = new DropoffPointsWs();
...
//        $check_pending = $farmer->get_farmer_rsbsa($rsbsa_control_no);
    ...
    public function search_FarmerWs() {
        $term = $_GET['term'];

        $results = array();

        $farmers = new FarmerWs();
...
//            $remaining_bags = 0;
    ...
    public function search_address($farmer_id) {
        ...
        // Get provinces
        $provinces = new ProvincesWs();
        $provinces_list = $provinces->all_ProvincesWs();
