                    ...
                    @foreach ($regions as $region)
                        <div class="card">
                            <div class="card-header" id="headingOne">
                                <h5 class="mb-0" style="margin:0" data-toggle="collapse"
                                    data-target="#collapse{{$region->id}}"
                                    onclick="getCoopsDetails({{$region->id}}, 'list_{{$region->id}}')">
                                    <button style="color: #7387a8;text-decoration:none;" class="btn btn-link">
                                        {{$region->regDesc}}
                                    </button>
                                    <i class="fa fa-plus pull-right" id="icon_id_{{$region->id}}"
                                        style="margin-top: 12px;margin-right: 10px;" data-toggle="collapse"
                                        data-target="#collapse{{$region->id}}" aria-controls="{{$region->id}}"
                                        onclick="getCoopsDetails({{$region->id}}, 'list_{{$region->id}}')"></i>
                                </h5>
                            </div>
                            <div id="collapse{{$region->id}}" class="collapse" aria-labelledby="headingOne"
                                data-parent="#accordion" style="margin-top: .5vw;">
                                <div class="card-body">
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery.each(data, function (index, array_value) {
                        ...
                        var export_btn_py = '<button id="' + button_id_py + '" onclick="exportDeliveriesPy(' + coop_accreditation_py + ')" class="btn btn-success btn-sm form-control"><i class="fa fa-table"></i> Export Deliveries (w/ DV)</button>';
        ...
        function exportDeliveriesPy(coop_accreditation, button_id_py) {
            ...
            $.ajax({
                ...
                success: function (response, textStatus, request) {
                    ...
                    $("#" + button_id_py).empty().html('<i class="fa fa-table"></i> Export Deliveries (w/ DV)');
                }
            });
        }
