        ...
        <div class="x_content form-horizontal form-label-left">
            <br/>
                <table class="table table-hover table-striped table-bordered" id="region_tbl">
                    ...
                    <tbody>
                        @foreach($provincial_data as $province)
                            <tr>
                                <td>{{$province->province}}</td>
                                <td>{{$province->total_farmers == '' ? 0 : $province->total_farmers}}</td>
                                <td>{{$province->total_dist_area == '' ? 0 : $province->total_dist_area}}</td>
                                <td>{{$province->total_actual_area == '' ? 0 : $province->total_actual_area}}</td>
                                <td>{{$province->total_bags == '' ? 0 : $province->total_bags}}</td>
                                <td>{{$province->total_male == '' ? 0 : $province->total_male}}</td>
                                <td>{{$province->total_female == '' ? 0 : $province->total_female}}</td>
                                <td><a class="btn btn-success btn-sm" href="{{ route('rcef.report.excel.province', $province->province) }}"><i class="fa fa-calendar"></i> GENERATE EXCEL</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
