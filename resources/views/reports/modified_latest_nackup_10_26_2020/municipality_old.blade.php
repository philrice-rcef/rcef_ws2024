                    ...
                    <tbody>
                        @foreach($municipal_data as $row)
                            <tr>
                                ...
                                <td>{{$row->total_farmers == '' ? 0 : $row->total_farmers}}</td>
                                <td>{{$row->total_dist_area == '' ? 0 : $row->total_dist_area}}</td>
                                <td>{{$row->total_actual_area == '' ? 0 : $row->total_actual_area}}</td>
                                <td>{{$row->total_bags == '' ? 0 : $row->total_bags}}</td>
                                <td>{{$row->total_male == '' ? 0 : $row->total_male}}</td>
                                <td>{{$row->total_female == '' ? 0 : $row->total_female}}</td>
                                <td><a class="btn btn-success btn-sm" href="{{ route('rcef.report.excel.municipality', ['province' => $row->province, 'municipality' => $row->municipality]) }}"><i class="fa fa-calendar"></i> GENERATE EXCEL</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
