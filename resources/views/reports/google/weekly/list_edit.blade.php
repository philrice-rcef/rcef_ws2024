...
@section('content')
    ...
    <div class="col-md-12 col-sm-12 col-xs-12" style="height: 1200px;">
        ...
        <form action="{{route('rcep.google_sheet.weeklyUpdate')}}" method="POST" data-parsley-validate="" id="actual_form">
            {{ csrf_field() }}
            <div class="x_panel">
                <div class="x_title">
                    <h2>
                        GOOGLE SHEETS | WEEKLY REPORT | EDIT
                    </h2>

                    <button role="submit" type="submit" class="btn btn-warning pull-right" style="border-radius:20px;"><i class="fa fa-edit"></i> SAVE CHANGES</button>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                    <div class="form-horizontal form-label-left">
                            ...
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <select name="province" id="province" class="form-control">
                                    ...
                                    @foreach ($provinces as $to_province)
                                        @if($to_province->province == $report_province)
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <select name="municipality" id="municipality" class="form-control">
                                    <option value="{{$report_municipality}}">{{$report_municipality}}</option>
                                </select>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <input type="text" name="date_range" id="date_range" class="form-control" value="{{$report_date_from}} - {{$report_date_to}}" />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-2 col-sm-2 col-xs-2"><span style="color: red">*</span>Bags Distributed:</label>
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <input type="number" name="bags_distributed" id="bags_distributed" class="form-control" value="{{$report_bags_distributed}}" min="1" max="99999">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-2 col-sm-2 col-xs-2"><span style="color: red">*</span>Farmer Beneficiaries:</label>
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <input type="number" name="farmer_beneficiaries" id="farmer_beneficiaries" class="form-control" value="{{$report_farmer_beneficiaries}}" min="1" max="99999">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-2 col-sm-2 col-xs-2"><span style="color: red">*</span>Area planted (ha):</label>
                            <div class="col-md-10 col-sm-10 col-xs-10" required>
                                <input type="number" name="area_planted" id="area_planted" class="form-control" value="{{$report_area_planted}}" min="1" max="99999" step=".01">
                            </div>
                        </div>
                    </div>
                </div>
