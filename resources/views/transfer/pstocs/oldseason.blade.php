...
<div class="page-title">
    <div class="title_left" style="width: 100%;">
        <h3>PREVIOUS SEASON TO CURRENT SEASON TRANSFER
            <br>        
        </h3>
        [{{$coop_name}}]
    </div>
...
<div class="row">
    <div class="col-md-6">
        <div class="x_panel">
            <div class="x_title">
                <h2>[{{$batch_number}}] DELIVERY DETAILS <br>
            ...
            <div class="x_content form-horizontal form-label-left">
                <div class="form-group">
                    <div class="row">
						...
						<div class="col-md-9">
                            <br>
                            <input type="radio" name="seedType" id="inventory" checked="checked" value="I"> <label for="inventory"> Inventory Seeds </label> &emsp;
                            <input type="radio" name="seedType" id="buffer" value="B"> <label for="buffer"> Buffer Seeds </label> 
                        </div>
                    </div>
                </div>
                <div class="form-group">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">[{{$batch_number}}] CONFRIM TRANSFER (PREVIOUS SEASON - CURRENT SEASON) 
                            <br>
                            {{$coop_name}}
                    ...
                    <div class="modal-body" style="max-height: 400px;overflow-y: auto;">
                        ...
                        <input type="hidden" name="coop_acre" id="coop_acre" value="{{$coop_acre}}">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="go_pstocs_transfer"><i class="fa fa-check-circle"></i> CONFIRM TRANSFER</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
   ...
   // $("#seed_tag_list").select2();
        ...
        jQuery.each(exploded_transferred_str, function(index, array_value){
            ...
            if(array_value != ""){
                var split_str = array_value.split("&");
					var seedTypeInfo = "";
                    if (split_str[2] === 'I')seedTypeInfo = "Inventory Seeds";
                    else if (split_str[2] === 'B')seedTypeInfo = "Buffer Seeds";
				
                table_str = table_str + '<tr>';
                table_str = table_str + '<td>'+split_str[0]+'</td>';
				table_str = table_str + '<td>'+seedTypeInfo+'</td>';
    ...
    $("#transfer_municipality").on("change", function(e){
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('transfers.dropoff.pstocs') }}",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    $("#add_to_selection_btn").on("click", function(e){
		...
		var seedType =  $('input[name="seedType"]:checked').val();
        ...
        if(seed_tag_value != "" && seed_tag != "0" || seed_tag_value != 0 && seed_tag != "0"){
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('transfers.check_seedtag.oldseason') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(data){
                    if(data["msg"] == "amount_to_transfer_ok"){
                        $("#seed_tag_str_temp").val(current_seedtag_str + seed_tag + "&" + seed_tag_value + "&" + seedType + "*|*");
                        $('#seed_tag_list option:selected').remove();

							var seedTypeInfo = "";
                            if(seedType === 'I')seedTypeInfo = "Inventory Seeds";
                            else if (seedType === 'B')seedTypeInfo = "Buffer Seeds";						
                        var card_str = '';
                        ...
                        card_str = card_str + '<h5 class="mb-0" style="margin:0">';
                        card_str = card_str + '<button style="color: #7387a8;text-decoration:none;" class="btn btn-link">'+seed_tag+' - '+seed_tag_value+' bag(s) | '+seedTypeInfo+'</button>';
    ...
    function removeTagFromList(index, seed_tag_value){
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('transfers.seed_tag.details.oldseason') }}",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    $("#go_pstocs_transfer").on("click", function(e){
          HoldOn.open(holdon_options);
        var province_name = $("#transfer_province option:selected").text();
        var municipality_name = $("#transfer_municipality option:selected").text();
        var dop_id = $("#transfer_dop").val();
        var batch_number = $("#batch_number").val();
        var coop_acre = $("#coop_acre").val();
        var transfer_str = $("#seed_tag_str_temp").val();
		
 /*
        $.ajax({
            type: 'POST',
            url: "{{ route('confirm.transfer.pstocs') }}",
            data: {
                _token: "{{ csrf_token() }}",
                batch_number: batch_number,
                coop_acre: coop_acre,
