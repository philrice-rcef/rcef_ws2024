							...
							<div class="row tile_count" style="margin: 0">
								...
								<table class="table table-bordered table-striped" id="tbl_encoderStatus">
									<thead>
										...
										<th>Indicative Target</th>
										<th>No. of Encoded</th>
										<th>LAND PREPARATION</th>
          								<th>SEEDBED PREPARATION</th>
          								<th>SEEDLING TRAY PREPARATION</th>
          								<th>SEEDLING MANAGEMENT</th>
          								<th>SEEDBED FERTILIZATION</th>
          								<th>DIRECT SEEDING</th>
          								<th>Transplanting</th>
          								<th>Mechanized Planting</th>
          								<th>Fertilizer Management</th>
          								<th>Water Management</th>
          								<th>Pest Management</th>
          								<th>Weed Management</th>
          								<th>Harvest Management</th>
...
@push('scripts')
	...
	function savedata(){
		...
		if(ntencode<=0){
			alert("Please Input Number in Text Box")
				...
				$.ajax({
            	type: "POST",
            	url: "{{url('palaysikatan/palaysikatan.station-status-list-encoded')}}",
            	data: {
            	    _token: "{{ csrf_token() }}",
            	    username: username,
            	    ntencode: ntencode,
	...
	function encoder_load_tbl(station, encoder) {
            ...
            $("#tbl_encoderStatus").DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "{{ route('palaysikatan.station-status-list-encoded') }}",
                    "dataType": "json",
                    "type": "POST",
                    "data": {
                        "_token": "{{ csrf_token() }}",
                ...
                "columns": [
                    ...
                    {"data":"seedbed_preparation"},

                    {"data":"seedling_try_preparation"},

                    {"data":"seedling_management"},

                    {"data":"seedbed_fertilization"},

                    {"data":"crop_establishment"},

                    {"data":"ce_transplanting"},

                    {"data":"ce_mechanized_planting"},

                    {"data":"fertilized_management"},

                    {"data":"water_management"},

                    {"data":"pest_management"},

                    {"data":"weed_management"},

                    {"data":"harvest_management"},
