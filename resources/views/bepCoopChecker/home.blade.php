...
@section('content')
    ...
    <div class="row" id="page2">
        <div class="col-md-12" id="page1">
            ...
            <div class="x_title">
                <h1>BeP Claims Cooperative Checker</h1>
...
@push('scripts')
    ...
    <script> 
        ...
        $('#tagBepCoopChecker').removeAttr('disabled');
        ...
        $('#region').on('change', () => {
            ...
            $.ajax({ 
                type: 'POST',
                url: "{{ route('getBepCoopCheckerProvinces') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    reg: $reg
                },
        ...
        $('#provinces').change(() => {
            ...
            $.ajax({ 
                type: 'POST',
                url: "{{ route('getBepCoopCheckerMunicipalities') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    prov: $prov
                },
        ...
        $('#submit').on('click', () =>{
            ...
            else if($muni == 'default')
            {
                ...
                $.ajax({ 
                type: 'POST',
                url: "{{ route('updateCoops') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    prov: $prov
                },
                success: function(response){
                    alert('Updated ' + response + ' entries');
                    HoldOn.close();
